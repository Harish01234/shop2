...
type AdvanceSearchFilterProps = {
  ...
  onChange: (filters: InterestFilterValues) => void
  totalCount: number
}
...
function rangeTip(filters: InterestFilterValues) {
...
function isDateChip(key: keyof InterestFilterValues) {
...
export function AdvanceSearchFilter({
  ...
  const [draft, setDraft] = useState<InterestFilterValues>(filters)
  const filterCount = countActiveFilters(filters)
  const chips = interestFilterChips(filters)
  const rangeLabel = rangeTip(filters)
  const countTip = rangeLabel
    ? `${totalCount} Interest ${rangeLabel}`
    : `${totalCount} Interest`
  ...
  function updateDraft<Key extends keyof InterestFilterValues>(
    key: Key,
    value: InterestFilterValues[Key] | undefined,
  ) {
            ...
            <Card className="shadow-none ring-foreground/10">
              ...
              <CardContent>
                <FieldGroup className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <Field>
                    <FieldLabel htmlFor="filter-personName">Person</FieldLabel>
                    <Input
                      id="filter-personName"
                  ...
                  <Field>
                    <FieldLabel htmlFor="filter-amountMin">Amount min</FieldLabel>
                    <Input
                      id="filter-amountMin"
                      type="number"
                      min="0"
                  ...
                  <Field>
                    <FieldLabel htmlFor="filter-amountMax">Amount max</FieldLabel>
                    <Input
                      id="filter-amountMax"
                      type="number"
                      min="0"
