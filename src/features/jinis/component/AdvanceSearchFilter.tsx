...
type AdvanceSearchFilterProps = {
  ...
  onChange: (filters: JinisFilterValues) => void
  totalCount: number
  activeCount: number
}
...
function rangeTip(filters: JinisFilterValues) {
...
function isDateChip(key: keyof JinisFilterValues) {
...
export function AdvanceSearchFilter({
  ...
  const [draft, setDraft] = useState<JinisFilterValues>(filters)
  const filterCount = countActiveFilters(filters)
  const chips = jinisFilterChips(filters)
  const rangeLabel = rangeTip(filters)
  const countTip = rangeLabel
    ? `${activeCount} active of ${totalCount} Jinis ${rangeLabel}`
    : `${activeCount} active of ${totalCount} Jinis`
  ...
  function updateDraft<Key extends keyof JinisFilterValues>(
    key: Key,
    value: JinisFilterValues[Key] | undefined,
  ) {
          ...
          <Card className="shadow-none ring-foreground/10">
            ...
            <CardContent>
              <FieldGroup className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Field>
                  <FieldLabel htmlFor="filter-fatherName">Father's Name</FieldLabel>
                  <Input
                ...
                <Field>
                  ...
                  <NativeSelect
                    ...
                    onChange={(event) =>
                      updateDraft(
                        'type',
                        isJinisType(event.target.value)
                          ? event.target.value
                          : undefined,
                      )
                    }
